...
local function OnPlayerCommandWithArg(event, player, code)
    if(string.find(code, " "))then
		...
		if (arguments[1] == "addtodd" and #arguments == 2 ) then
			local name = arguments[2]
			local query = CharDBQuery("SELECT * FROM characters.dd_chars WHERE name = '"..name.."'")
			if query then
				player:SendBroadcastMessage("Игрок "..name.." УЖЕ добавлен в список участников Дня Дракона.")
			else
				player:SendBroadcastMessage("Игрок "..name.." успешно ДОБАВЛЕН в список участников Дня Дракона!")
				CharDBExecute("INSERT INTO `characters`.`dd_chars` (`name`) VALUES ('"..name.."');")
			end
		elseif (arguments[1] == "removefromdd" and #arguments == 2 ) then
			local name = arguments[2]
			local query = WorldDBQuery("SELECT * FROM characters.dd_chars WHERE name = '"..name.."'")
			if query then
				player:SendBroadcastMessage("Игрок "..name.." успешно ИСКЛЮЧЕН из списка участников Дня Дракона!")
				CharDBExecute("DELETE FROM `characters`.`dd_chars` WHERE  `name`='"..name.."' LIMIT 1;")
			else
				player:SendBroadcastMessage("Игрок "..name.." НЕ НАХОДИТСЯ в списке участников Дня Дракона.")
			end
		end
	end
end
