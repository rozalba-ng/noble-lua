...
local ENTRY = 987820
local ITEM_ENTRY = 301345
local FOOD_ENTRY = 5059139
local LOOT_ENTRY = 2592
local FOOD_COUNT = 3

local TYPE = 2
local info = {max_stage = 13, dirt_chance=0.05, food_every = 4, entry = ENTRY}
...
local function Interface_Collect(player,animal_object,intid)
	...
	local item = player:AddItem(LOOT_ENTRY,20)
	if item then
		animal:RestartCycle()
	else
...
local function OnAnimalTalk(event, player, animal_object)
	...
	if animal.health > 0 then
		...
		if animal.want_clean == 0 and animal.want_eat == 0 and animal.stage == info.max_stage then
			interface:AddRow("Состричь шерсть",Interface_Collect, true):SetIcon(5)
	...
	if animal.health > 0 then
		local collectText = animal.stage < info.max_stage and ""..animal.stage.."/"..info.max_stage or "Можно стричь"
		title = title.."\nПрогресс шерсти: "..collectText
	...
	if nearestFarm then
		...
		if house then
			if house:HasAccess(player) then
				...
				FarmSystem.InitNewAnimal(animal_object,house.gob_guid,TYPE,"Овца")
