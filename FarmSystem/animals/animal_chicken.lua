...
local ENTRY = 987818
local ITEM_ENTRY = 301344
local FOOD_ENTRY = 5059140
local LOOT_ENTRY = 6889
local FOOD_COUNT = 3

local TYPE = 4
local info = {max_stage = 9999, dirt_chance=0.053, food_every = 6, entry = ENTRY}
FarmSystem.AnimalsInfo[TYPE] = info


local function Interface_Collect(player,animal_object,intid,count)
	local animal = FarmSystem.GetAnimal(animal_object)
	local item = player:AddItem(LOOT_ENTRY,count)
	if item then
		animal:RestartCycle()
	else
...
local function OnAnimalTalk(event, player, animal_object)
	...
	if animal.health > 0 then
		if animal.want_eat == 1 then
			interface:AddRow("Накормить стручками кукурузой", FarmSystem.Interface_FeedAnimal, true,nil,FOOD_ENTRY, FOOD_COUNT,"Стручок кукурузы"):SetIcon(5)
		end
		if animal.want_clean == 1 then
			interface:AddRow("Почистить перья", FarmSystem.Interface_CleanAnimal, true):SetIcon(5)
		end
		if animal.stage > 0 then
			interface:AddRow("Собрать яйца",Interface_Collect, true,nil,animal.stage):SetIcon(5)
	...
	if animal.health > 0 then
		local collectText = animal.stage == 0 and "Ни одного"or animal.stage .." шт."
		title = title.."\nЯйца: "..collectText
		title = title.."\nЗдоровье: "..animal.health.."/100"
		local cleanText = animal.want_clean == 1 and "Грязная" or "Чистая"
		title = title.."\nГигиена: "..cleanText
		local eatText = animal.want_eat == 1 and "Голодает" or "Сытая"
		title = title.."\nПитание: "..eatText
	...
	if nearestFarm then
		...
		if house then
			if house:HasAccess(player) then
				...
				FarmSystem.InitNewAnimal(animal_object,house.gob_guid,TYPE,"Курочка")
