...
local ENTRY = 987819
local ITEM_ENTRY = 301346
local FOOD_ENTRY = 5059139
local LOOT_ENTRY = 520065
local FOOD_COUNT = 5

local TYPE = 1
local info = {max_stage = 15, dirt_chance=0.1, food_every = 3, entry = ENTRY}
...
local function Interface_Collect(player,animal_object,intid)
	...
	local item = player:AddItem(LOOT_ENTRY,15)
	if item then
		animal:RestartCycle()
	else
...
local function OnAnimalTalk(event, player, animal_object)
	...
	if animal.health > 0 then
		if animal.want_eat == 1 then
			interface:AddRow("Накормить пшеницей", FarmSystem.Interface_FeedAnimal, true,nil,FOOD_ENTRY, FOOD_COUNT,"Пшеница"):SetIcon(5)
		end
		...
		if animal.want_clean == 0 and animal.want_eat == 0 and animal.stage == info.max_stage then
			interface:AddRow("Надоить молока",Interface_Collect, true):SetIcon(5)
	...
	if animal.health > 0 then
		local collectText = animal.stage < info.max_stage and ""..animal.stage.."/"..info.max_stage or "Можно доить"
		title = title.."\nПрогресс молока: "..collectText
	...
	if nearestFarm then
		...
		if house then
			if house:HasAccess(player) then
				...
				FarmSystem.InitNewAnimal(animal_object,house.gob_guid,TYPE,"Корова")
