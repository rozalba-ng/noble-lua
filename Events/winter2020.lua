...
local function Stage7( event, player, creature, sender, intid, code )
	if event == 1 then
		...
		if ( GetQuestStage(player) >= 18 and GetQuestStage(player) <= 20 ) then
			text = text.."\nА вам что нужно? Тоже задание проходите? Гипер сложное, говорите? Что уж тут теперь поделать... Но я могу перенести вас на следующий этап, конечно. Если у вас есть СЕКРЕТНЫЙ КОД. И не вздумайте спрашивать его у меня в дискорде! Нет, я серьёзно. Не надо.."
