...
local function OnGossipGoblin(event, player, object)
    ...
    if(result ~= nil) then
        ...
        player:GossipSetText( 'Хей-хей! Я вдоль и поперёк исследовал весь Азерот и уже успел оценить масштабы земель у Черной Горы. И знаешь что? Тебе ОПРЕДЕЛЁННО понадобится четвероногий приятель! Я, так и быть, готов расщедриться и выдать лично тебе одного из моих четырех любимцев. \n\n Но помни два условия — выбрать можно только одного из них и НИКАКИХ возвратов! Итак, кого ты выберешь? \n\n [ООС примечание: вы можете выбрать 1 маунта на аккаунт, но он будет доступен всем вашим персонажам. Выбирайте с умом!]', menuId )
    else
